...
function makeEntries(count: number, date: Date): CalendarEntry[] {
  return Array.from({ length: count }, (_, i) => ({
    id: `entry-${i}`,
    title: `Entry ${i + 1}`,
    startDate: new Date(date.getFullYear(), date.getMonth(), date.getDate(), 9 + i, 0),
    endDate: new Date(date.getFullYear(), date.getMonth(), date.getDate(), 10 + i, 0),
    wholeDay: false,
  }))
}
...
describe("MonthView", () => {
  ...
  it("clicking empty cell opens entry modal at 9am", () => {
    ...
    const cell15 = cells.find((cell) => cell.querySelector("span")?.textContent === "15")
